...
consulta indice armarios
  ...
  | fst (fromJust status) == Alugado = Just Alugado
  | otherwise = Just Livre
  where
      status = Map.lookup indice armarios
...
pegarArmario indice armarios
                | status == Nothing  || armario == Nothing = Left $ "Armario "++ show indice++ " nao existe"
                | otherwise = Right (fromJust armario)
                where
                    status = consulta indice armarios
...
pegarCodigo indice armarios
                | armario1 == Nothing = Left $ "Armario "++show indice++ " nao existe"
                | fromJust status == Alugado = Left $ "Armario "++ show indice ++" ja esta ocupado"
                | otherwise = Right $ snd(fromJust armario1)
                where
                    armario = pegarArmario indice armarios
                    armario1 = Map.lookup indice armarios
                    status = consulta indice armarios
...
aluguel indice armarios
                | armario1 == Nothing = Left $ "Armario "++ show indice ++" nao existe"
                | fst (fromJust armario1) == Livre = Right $ Map.insert indice (Alugado, snd $ fromJust armario1) armarios
                | otherwise = Left $ "Armario "++show indice++" está ocupado"
                where
                    status = pegarCodigo indice armarios
                    armario1 = Map.lookup indice armarios
...
devolucao indice codigo armarios
                        | armario1 == Nothing = Left $ "Armario "++ show indice ++" nao existe"
                        | (snd $ (fromJust armario1)) /= codigo = Left "codigo incorreto"
                        | fst (fromJust armario1) == Alugado = Right $ Map.insert indice (Livre, snd $ fromJust armario1) armarios
                        | otherwise = Left $ "Armario "++show indice++"está livre"
                        where
                            status = pegarCodigo indice armarios
                            armario1 = Map.lookup indice armarios
